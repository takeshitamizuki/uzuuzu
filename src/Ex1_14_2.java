...
public class Ex1_14_2 {
	...
	*/
	static int discount(int totalPrice){ //アンダーバーを適切な内容に書き換えてください
		int DISCOUNT_OVER = 1000;
		int DISCOUNT_BORDER = 5000;
		int DISCOUNT_MAX = 5000;
		double DISCOUNT_RATE = 0.1;
		double DISCOUNT_HIRATE = 0.2;
		int priceDownTemp = 0;
		if (totalPrice > DISCOUNT_OVER && totalPrice <= DISCOUNT_BORDER) {
			priceDownTemp = (int)((totalPrice - DISCOUNT_OVER) * DISCOUNT_RATE);
		} else if (totalPrice > DISCOUNT_BORDER) {
			priceDownTemp = (int)((DISCOUNT_BORDER - DISCOUNT_OVER) * DISCOUNT_RATE + (totalPrice - DISCOUNT_BORDER) * DISCOUNT_HIRATE);
		}
		int priceDown = 0;
		if (DISCOUNT_MAX < priceDownTemp) {
			priceDown = DISCOUNT_MAX;
		} else {
